...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Edit BOM: {{ $bom->name }}</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('boms.index') }}">BOMs</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('boms.show', $bom) }}">{{ $bom->name }}</a></li>
                ...
                <div class="card">
                    <div class="card-header border-bottom-0">
                        <h3 class="card-title">
                            ...
                            Edit Bill of Material: {{ $bom->name }}
                        </h3>
                        <div class="card-tools">
                            <a href="{{ route('boms.show', $bom) }}" class="btn btn-sm btn-outline-secondary">
                                <i class="fas fa-arrow-left mr-1"></i> Back to BOM
                            </a>
                        </div>
                    </div>

                    <form action="{{ route('boms.update', $bom) }}" method="POST" id="bomForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $bom->name) }}" required placeholder="Enter BOM name">
                                        @error('name')
                                        <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Enter BOM description">{{ old('description', $bom->description) }}</textarea>
                                @error('description')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                                @enderror
                            </div>
                                    ...
                                    <div class="bom-item-row mb-3 p-3 border rounded">
                                        <div class="row align-items-end">
                                            ...
                                            <div class="col-md-3">
                                                ...
                                                <input type="number" name="items[{{ $index }}][quantity]" required min="1" step="0.001" class="form-control quantity-input" value="{{ old("items.{$index}.quantity", $bomItem->quantity) }}">
                                            </div>
                                            <div class="col-md-2">
                                                <label class="form-label">Estimated Cost</label>
                                                <input type="text" readonly class="form-control cost-input bg-light" value="Rs {{ number_format($bomItem->quantity * $bomItem->item->current_price, 2) }}">
                                            </div>
                                            <div class="col-md-1">
                                                <button type="button" class="btn btn-danger remove-bom-item" {{ $loop->first ? 'disabled' : '' }}>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            ...
                                            <span id="sparePartsCount" class="font-weight-bold text-primary">{{ $bom->bomItems->count() }}</span>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="text-right">
                                            <h4 class="font-weight-bold text-dark">Total Estimated Cost</h4>
                                            <h2 id="totalEstimatedCost" class="font-weight-bold text-success">Rs {{ number_format($bom->total_estimated_cost, 2) }}</h2>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input" id="is_active" name="is_active" value="1" {{ old('is_active', $bom->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="card-footer bg-white border-top">
                            <div class="row justify-content-between align-items-center">
                                ...
                                <div class="col-md-6 text-right">
                                    <a href="{{ route('boms.show', $bom) }}" class="btn btn-outline-secondary mr-2">
                                        <i class="fas fa-times mr-1"></i> Cancel
                                    </a>
                                    <button type="submit" class="btn btn-primary">
                                        <i class="fas fa-save mr-1"></i> Update BOM
                                    </button>
                                </div>
                            </div>
                        </div>
