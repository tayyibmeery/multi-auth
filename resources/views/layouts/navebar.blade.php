			        ...
			        <div class="flex items-center justify-between py-4">
			            <div class="flex items-center space-x-4">
			                ...
			                <div class="hidden space-x-4 md:flex">
			                    ...
			                    <a href="{{ route("vendors.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Vendors</a>
			                    <a href="{{ route("categories.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Categories</a>
			                    @endif

			                    <a href="{{ route("items.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Spare Parts </a>

			                    <a href="{{ route("products.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Products</a>
			                    <a href="{{ route("purchases.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Purchases</a>
			                    <a href="{{ route("boms.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Bills of
			                        Materials</a>
			                    <a href="{{ route("production-runs.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Production</a>
			                    <a href="{{ route("stock-ledgers.index") }}" class="rounded px-3 py-2 hover:bg-blue-700">Stock
			                        Ledger</a>

			                    @if (auth()->user()->isAdmin())
			                    <a href="{{ route("reports.stock") }}" class="rounded px-3 py-2 hover:bg-blue-700">Reports</a>
			                    @endif
			                </div>
			                @endauth
			            </div>
			            ...
			            <div class="flex items-center space-x-4">
			                ...
			                @if (auth()->user()->isAdmin())
			                <a href="{{ route("admin.dashboard") }}" class="rounded bg-blue-700 px-3 py-2 hover:bg-blue-800">
			    ...
			    <div class="container mx-auto px-4 py-2">
			        <div class="grid grid-cols-4 gap-2 text-sm">
			            ...
			            <a href="{{ route("items.index") }}" class="rounded p-1 text-center hover:bg-blue-600">
			                ...
			                <span class="text-xs">Spare Parts </span>
