        ...
        <form action="{{ route("boms.update", $bom) }}" method="POST" class="p-6" id="bomForm">
            ...
            <div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    <select name="product_id" id="product_id" required class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" required class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500" value="{{ old("name", $bom->name) }}">
                    @error("name")
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500">{{ old("description", $bom->description) }}</textarea>
            </div>

            <!--  Bill of Materials spare parts -->
            <div class="mb-6">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-medium text-gray-900">BOM Spare Parts </h3>

                    <button type="button" id="addBomItem" class="rounded-lg bg-green-600 px-3 py-2 text-sm text-white hover:bg-green-700">
                        <i class="fas fa-plus mr-1"></i> Add Spare Parts
                ...
                <div id="bom-items-container">
                    ...
                    <div class="bom-item-row mb-4 grid grid-cols-12 items-end gap-4">
                        <div class="col-span-6">
                            ...
                            <select name="items[{{ $index }}][item_id]" required class="item-select mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500">
                                <option value="">Select Spare Part</option>

                                @foreach ($items as $item)
                                <option value="{{ $item->id }}" data-price="{{ $item->current_price }}" {{ old("items.$index.item_id", $bomItem->item_id) == $item->id ? "selected" : "" }}>
                                    {{ $item->name }} ({{ $item->code }}) - Stock:
                                    {{ $item->current_stock }} - Rs
                        ...
                        <div class="col-span-3">
                            ...
                            <input type="number" name="items[{{ $index }}][quantity]" required min="1" step="0.001" class="quantity-input mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500" value="{{ old("items.$index.quantity", $bomItem->quantity) }}">
                        </div>
                        <div class="col-span-2">
                            <label class="block text-sm font-medium text-gray-700">Estimated Cost</label>
                            <input type="text" readonly class="cost-input mt-1 block w-full rounded-md border border-gray-300 bg-gray-100 px-3 py-2 shadow-sm" value="Rs {{ number_format($bomItem->quantity * $bomItem->item->current_price, 2) }}">
                        </div>
                        <div class="col-span-1">
                            <button type="button" class="remove-bom-item rounded-lg bg-red-600 p-2 text-white hover:bg-red-700">
            ...
            <div class="mb-6 rounded-lg bg-gray-50 p-4">
                <div class="flex items-center justify-between">
                    ...
                    <span id="totalEstimatedCost" class="text-2xl font-bold text-blue-600">Rs
                        {{ number_format($bom->total_estimated_cost, 2) }}</span>
                </div>
            </div>

            <div class="mb-6 flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old("is_active", $bom->is_active) ? "checked" : "" }} class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500">
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route("boms.show", $bom) }}" class="rounded-lg bg-gray-300 px-4 py-2 font-medium text-gray-800 hover:bg-gray-400">
                    Cancel
                </a>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Add event listeners to a Bill of Materials Spare Parts row
