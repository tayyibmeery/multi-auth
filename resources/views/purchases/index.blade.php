...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('purchases.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium">
        ...
        <div class="bg-white p-4 rounded-lg shadow">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-lg font-semibold">{{ \App\Models\Purchase::whereMonth('purchase_date', now()->month)->count() }}</p>
                </div>
            </div>
        </div>
    </div>
                    ...
                    @forelse($purchases as $purchase)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-sm text-gray-500">{{ $purchase->purchase_date->format('M j, Y') }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('purchases.show', $purchase) }}" class="text-blue-600 hover:text-blue-900 mr-3">
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Monthly Purchase Summary ({{ now()->year }})</h3>
        @php
            ...
            <div class="space-y-4">
                @foreach($monthlyData as $data)
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-gray-600 w-24">
                            {{ DateTime::createFromFormat('!m', $data->month)->format('F') }}
                        </span>
                        <div class="flex-1 mx-4">
                            <div class="w-full bg-gray-200 rounded-full h-3">
                                <div class="bg-blue-600 h-3 rounded-full"
                                     style="width: {{ min(($data->total / ($monthlyData->max('total') ?: 1)) * 100, 100) }}%"></div>
                            </div>
                        </div>
                        <div class="text-right w-32">
                            <span class="text-sm font-medium text-gray-900">₹{{ number_format($data->total, 2) }}</span>
                            <span class="text-xs text-gray-500 ml-2">({{ $data->count }} purchases)</span>
                        </div>
                    </div>
                @endforeach
            </div>
                ...
                @foreach($topVendors as $vendor)
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-semibold text-blue-600">
                            ₹{{ number_format($vendor->purchases_sum_total_amount, 2) }}
                        </span>
                    </div>
                @endforeach
            </div>
