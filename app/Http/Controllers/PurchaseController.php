...
class PurchaseController extends Controller
{
    ...
    {
        // Load the purchase with its items and vendor
        $purchase->load('purchaseItems.item', 'vendor');
    ...
    public function update(Request $request, Purchase $purchase)
    {
        ...
        DB::transaction(function () use ($request, $purchase) {
            // First, reverse the stock changes from the original purchase
            foreach ($purchase->purchaseItems as $purchaseItem) {
                $item = $purchaseItem->item;
                $item->current_stock -= $purchaseItem->quantity;
                $item->save();

                // Remove old stock ledger entries
                StockLedger::where('reference_type', Purchase::class)
            ...
            foreach ($request->items as $itemData) {
                ...
                $item = Item::find($itemData['item_id']);
                $item->current_stock += $itemData['quantity'];
                $item->current_price = $itemData['unit_price'];
                $item->save();
