...
<div class="max-w-4xl mx-auto">
    <div class="bg-white shadow rounded-lg">
        ...
        <form action="{{ route('production-runs.store') }}" method="POST" class="p-6" id="productionForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="bom_id" id="bom_id" required
                        ...
                        @foreach($boms as $bom)
                            <option value="{{ $bom->id }}"
                                    data-product="{{ $bom->product->name }}"
                                    data-cost="{{ $bom->total_estimated_cost }}"
                                    {{ old('bom_id') == $bom->id ? 'selected' : '' }}>
                                {{ $bom->name }} - {{ $bom->product->name }} (Cost: ₹{{ number_format($bom->total_estimated_cost, 2) }})
                            </option>
                        @endforeach
                    </select>
                    @error('bom_id')
            ...
            <!-- Product Information -->
            <div id="productInfo" class="bg-blue-50 p-4 rounded-lg mb-6 hidden">
                <h4 class="text-sm font-medium text-blue-900 mb-2">Product Information</h4>
            ...
            <!-- Quantity -->
            <div class="mb-6">
                <label for="quantity_to_produce" class="block text-sm font-medium text-gray-700">Quantity to Produce *</label>
            ...
            <div id="bomItemsPreview" class="mb-6 hidden">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Required Materials</h3>
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div id="stockCheckResults"></div>
                    <table class="min-w-full divide-y divide-gray-200 mt-4 hidden" id="materialsTable">
            ...
            <!-- Total Cost -->
            <div id="totalCostSection" class="bg-gray-50 p-4 rounded-lg mb-6 hidden">
                <div class="flex justify-between items-center">
                    <span class="text-lg font-semibold text-gray-900">Total Material Cost:</span>
                    <span id="totalProductionCost" class="text-2xl font-bold text-blue-600">₹0.00</span>
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('production-runs.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg font-medium">
                    Cancel
                </a>
                <button type="submit" id="submitBtn" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium">
                    Start Production
                </button>
            </div>
        </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    bomSelect.addEventListener('change', function() {
        ...
        if (selectedOption.value) {
            ...
            materialCost.textContent = '₹' + parseFloat(cost).toFixed(2);
    ...
    function calculateTotalCost(items, quantity) {
        ...
        totalProductionCost.textContent = '₹' + totalCost.toFixed(2);
